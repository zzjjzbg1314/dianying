...
public class SignUtils {
	...
	public static boolean checkParam(Map<String, String> params, String key) {
		...
		if (params.containsKey("sign")) {
			...
			params.remove("sign");
			Map<String, String> map =  removeEmptyElement(params);
			StringBuilder buf = new StringBuilder((map.size() + 1) * 10);
			SignUtils.buildPayParams(buf, map, false);
			String preStr = buf.toString();
			String signRecieve = MD5.sign(preStr, "&key=" + key, "utf-8").toUpperCase();
			result = sign.equalsIgnoreCase(signRecieve);
		}
		return result;
	}
	
	private static Map<String, String> removeEmptyElement(Map<String, String> oldMap){
		Map<String, String> map = new HashMap<String, String>();
		if (null != oldMap) {
			for(String keyStr : oldMap.keySet()){
				String value  = oldMap.get(keyStr);
	...
	public static void buildPayParams(StringBuilder sb, Map<String, String> payParams, boolean encoding) {
		...
		for (String key : NewKeys) {
			for (String payKey : payParams.keySet()) {
				if (key.equals(payKey.toLowerCase())) {
					sb.append(key).append("=");
					if (encoding) {
						sb.append(urlEncode(payParams.get(payKey)));
					} else {
						sb.append(payParams.get(payKey));
					}
					sb.append("&");
					continue;
				}
